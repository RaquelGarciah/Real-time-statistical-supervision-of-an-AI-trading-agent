...
Las dos secciones anteriores dejan un hecho sin explicar: qué canal rescata cada activo cambia de uno a otro. Lo abordamos en tres pasos. Separamos las funciones de las dos capas, medimos qué propiedad del activo ordena el rescate del aprendiz, y dejamos que un clustering sin etiquetas recupere esos grupos. Sobre diez activos esto es un patrón fuerte, no un teorema.

\subsection{Dos supervisores, dos funciones}
\label{sec:mp-mec-capas}

STRATA tiene dos capas y cada una rescata un eje distinto del agente, sin solaparse: cuando una lidera, la otra no (Tabla~\ref{tab:mp-capas}). La regla determinista (M8) es la capa de riesgo, apoyada entera en el régimen. Su P\&L de rescate sobre SPY, $+0{,}312$, sale al $100\,\%$ del canal RAM; PSA y GSO aportan cero. Agregado el panel, ese rescate se vuelve significativo: el $\Delta$Sharpe pooled de la regla frente al agente es $+0{,}60$ con IC$_{95}\,[0{,}05,\ 1{,}22]$, que excluye el cero. Y aun así la regla nunca lidera en accuracy: $0$ de $10$.

El aprendiz (M10 y AutoML) hace el trabajo inverso. Mejora la accuracy del agente con significancia (McNemar pareado frente a M5) y modela el signo del día siguiente mejor que la regla fija. No lidera el riesgo en exclusiva: su ventaja de Sharpe frente a la regla es no concluyente, y el TOST no separa a las dos capas en ese eje. Cada supervisor cubre el eje que el otro deja descubierto (Figura~\ref{fig:mp-capas}).
...
Si el aprendiz rescata la accuracy, queda preguntar si lo hace por igual en todos los activos. Hay un orden, y lo gobierna una sola propiedad: el rescate escala con el \emph{leverage effect}. Medimos el rescate como la mejora de accuracy del mejor aprendiz sobre el agente, $\max(\mathrm{M10},\mathrm{AutoML}) - \mathrm{M5}$, y la naturaleza del activo con la correlación de Black,
\[
...
Las dos magnitudes corren juntas (Tabla~\ref{tab:mp-ley}, Figura~\ref{fig:mp-scatter}): a \emph{leverage} más negativo, más rescate. Pearson $r = -0{,}56$ ($p = 0{,}093$), Spearman $\rho = -0{,}59$ ($p = 0{,}074$) sobre los diez; correlación fuerte y tendencia significativa al nivel $\alpha = 0{,}10$ pre-registrado. Nueve de los diez la siguen; ROKU es la excepción, con \emph{leverage} casi nulo y rescate alto. Partido el panel, los activos de \emph{leverage} fuerte reciben un rescate medio de $0{,}097$ y los de \emph{leverage} débil de $0{,}059$. Cuando el régimen es direccional, la regla ya disciplina el riesgo y al aprendiz le queda margen para afinar la dirección; cuando el régimen no informa el signo, el aprendiz rescata por otra vía y la ganancia se estrecha.

Con $n = 10$ no afirmamos $p < 0{,}05$ ni robustez \emph{leave-one-out}: el test con diez puntos es poco potente, y por eso lo presentamos como tendencia al $\alpha = 0{,}10$. La naturaleza ordena el rescate del aprendiz, pero no predice el de la regla: ninguna propiedad (\texttt{crisis\_mean}, \emph{leverage} o sesgo del agente) correlaciona con el rescate de la regla a ningún nivel razonable (todas con $p > 0{,}14$). El \texttt{crisis\_mean} es descriptivo. La única ley del rescate es la del \emph{leverage} sobre el aprendiz.
...
La ley del \emph{leverage} es una recta. Queda ver si esa misma estructura aparece cuando no le decimos al algoritmo qué buscar. Aparece: agrupar los diez activos por su naturaleza recupera los mismos bloques que el rescate dibuja, sin ver ni una métrica de rescate. Describimos cada activo por cinco rasgos de naturaleza (\emph{leverage}, retorno medio en Crisis, fracción de días en Crisis, volatilidad OOS y sesgo direccional del agente) y lo agrupamos con cuatro métodos: $k$-medias (MacQueen 1967 \cite{macqueen1967}; Lloyd 1982 \cite{lloyd1982}), enlace de Ward (Ward 1963 \cite{ward1963}), mezcla gaussiana (Dempster \emph{et al.} 1977 \cite{dempster1977}) y clustering espectral (Ng \emph{et al.} 2002 \cite{ng2002spectral}).

Los cuatro coinciden. Con tres grupos, el índice de Rand ajustado entre cualquier par de métodos es $1{,}0$ (Hubert y Arabie 1985 \cite{hubert1985}) y la silueta media es $0{,}55$: partición unánime y bien separada. Cada grupo trae asociado un canal de rescate distinto (Tabla~\ref{tab:mp-cluster}): los índices de \emph{leverage} fuerte (SPY, QQQ, XLF, DIA, XLK, XLE) rescatan por AutoML; los de \emph{leverage} invertido (SMCI, UNG), por M10; y los volátiles (ROKU, MARA), de nuevo por AutoML. Al proyectar la naturaleza de los diez sobre sus dos primeras componentes principales, el primer eje recupera casi exactamente el \emph{leverage}: la correlación entre PC1 y \texttt{leverage\_corr} es $r = 0{,}84$ (Figura~\ref{fig:mp-pca}). La naturaleza define un eje, ese eje es el \emph{leverage}, y el \emph{leverage} ordena qué canal rescata cada activo. La Figura~\ref{fig:mp-casos} lo aterriza en dos casos, XLE rescatado por el régimen y MARA por la vía del \emph{leverage} invertido.

Esto es exploratorio: con diez activos el clustering describe, no confirma. El cluster predice el \emph{tipo} de canal que rescata cada grupo, no el modelo exacto que ganará por activo; qué derivada concreta lidera puede cambiar dentro de un mismo grupo, y eso no es predecible con $n = 10$. La estructura es robusta como dibujo, no como regla de asignación.
