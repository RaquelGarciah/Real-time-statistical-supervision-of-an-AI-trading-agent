...
Las dos secciones anteriores dejan un hecho sin explicar: qué canal rescata cada activo cambia de uno a otro. Esta sección lo aborda en tres pasos encadenados. Separamos las funciones de las dos capas y vemos que ninguna hace el trabajo de la otra. Medimos después qué propiedad del activo ordena el rescate del aprendiz, y resulta ser el \emph{leverage effect}. Y dejamos que un clustering, sin etiquetas, recupere los mismos grupos que esa propiedad dibuja. La cadena naturaleza$\to$eje$\to$rescate se cierra así, con una cautela por delante: sobre diez activos esto es un patrón fuerte, no un teorema.
...
El cierre lo da la geometría. Al proyectar la naturaleza de los diez sobre sus dos primeras componentes principales, el primer eje recupera casi exactamente el \emph{leverage}: la correlación entre la primera componente y \texttt{leverage\_corr} es $r = 0{,}84$ (Figura~\ref{fig:mp-pca}). Sin etiquetas de rescate, el clustering ordena los activos por el mismo eje que la ley de la Sección~\ref{sec:mp-mec-ley}: la naturaleza define un eje, ese eje es el \emph{leverage}, y el \emph{leverage} ordena qué canal rescata cada activo. La Figura~\ref{fig:mp-casos} lo aterriza en dos casos trabajados, XLE rescatado por el régimen y MARA por la vía del \emph{leverage} invertido.
