...
La Tabla~\ref{tab:mp-calib} recoge cada decisión de calibración con su criterio. El número de estados del HMM, $K = 3$, sale de una selección de modelo ex ante: la log-verosimilitud held-out por observación mejora de $-1{,}69$ con $K = 2$ a $-1{,}30$ con $K = 3$, y el BIC acompaña ($18\,775$ frente a $24\,131$). El orden de los tres estados refleja lo que justifica su uso sobre SPY: el estado más volátil tiene el retorno medio de calibración más bajo y el más tranquilo el más alto. Esa gradación negativa entre volatilidad y retorno es la que vuelve el régimen un proxy de la dirección en un índice amplio (Rabiner 1989 \cite{rabiner1989}); los etiquetamos Calma, Estrés y Crisis. La volatilidad la modelamos con un GARCH(1,1) de innovaciones $t$ de Student: la distribución $t$ responde al exceso de curtosis de los retornos diarios de renta variable, que hace inadecuada la normalidad (Bollerslev 1986 \cite{bollerslev1986}). El cambio estructural lo seguimos con BOCPD en modo \emph{online}, que en cada paso actualiza la probabilidad sobre el instante del último cambio sin usar información posterior (Adams y MacKay 2007 \cite{adams2007}). Los tres umbrales son cuantiles de la calibración: RAM $\tau = 0{,}50$, PSA $P_{95} = 0{,}023$ y GSO $P_{95} = 2{,}371$.
...
La objeción más natural es que los umbrales se eligieran para que el resultado saliera bien. El barrido completo lo descarta (Figura~\ref{fig:mp-sensibilidad}). La accuracy de M8 frente al umbral RAM es plana en una meseta: $0{,}466$ en $\tau = 0{,}3$ y $0{,}478$ a partir de $\tau = 0{,}5$, sin saltos. La del meta-learner frente a su umbral de decisión también es plana en torno a $0{,}50$. El resultado no cuelga de un punto afortunado del espacio de hiperparámetros: descansa sobre una región estable, que es justo lo contrario de un ajuste fino al periodo de evaluación. Esta meseta es nuestra mejor defensa frente a la sospecha de sobreajuste.
