...
La Tabla~\ref{tab:mp-calib} recoge cada decisión de calibración con su criterio. El número de estados del HMM, $K = 3$, sale de una selección de modelo ex ante: la log-verosimilitud held-out por observación mejora de $-1{,}69$ con $K = 2$ a $-1{,}30$ con $K = 3$, y el BIC acompaña ($18\,775$ frente a $24\,131$). El orden de los tres estados justifica su uso sobre SPY: el estado más volátil tiene el retorno medio de calibración más bajo y el más tranquilo el más alto. Esa gradación negativa entre volatilidad y retorno vuelve el régimen un proxy de la dirección en un índice amplio (Rabiner 1989 \cite{rabiner1989}); los etiquetamos Calma, Estrés y Crisis. La volatilidad la modelamos con un GARCH(1,1) de innovaciones $t$ de Student, cuya cola pesada responde al exceso de curtosis de los retornos diarios de renta variable, que hace inadecuada la normalidad (Bollerslev 1986 \cite{bollerslev1986}). El cambio estructural lo seguimos con BOCPD en modo \emph{online}, que en cada paso actualiza la probabilidad sobre el instante del último cambio sin usar información posterior (Adams y MacKay 2007 \cite{adams2007}). Los tres umbrales son cuantiles de la calibración: RAM $\tau = 0{,}50$, PSA $P_{95} = 0{,}023$ y GSO $P_{95} = 2{,}371$.
...
La anatomía de esas ciento veintiuna intervenciones está en la matriz de confusión de la Figura~\ref{fig:mp-confusion-spy}. En setenta y un casos el agente apostaba contra el régimen y la intervención lo corrige; en cincuenta el agente tenía razón y la intervención mete ruido. El saldo neto es positivo porque la intervención no es indiscriminada: crece con la discrepancia entre la apuesta del agente y el signo del régimen. Cuanto más se aleja el agente del estado del mercado, con más fuerza actúa la regla, y son justo esos días de discrepancia grande los que aportan el grueso del rescate.
...
El rescate de SPY tiene una sola fuente. Todo el P\&L acumulado de la intervención, $+0{,}312$, sale de los días en que RAM dispara; PSA y GSO aportan exactamente cero, porque sobre este activo no se encienden como reglas. Sobre SPY el mecanismo es el canal de régimen y nada más.
