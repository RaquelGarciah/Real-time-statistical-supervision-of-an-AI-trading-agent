...
Desarrollo de la Proposición~\ref{prop:forward} (Recursión forward), Sección~\ref{sec:hmm-forward}.
...
Desarrollo de la Proposición~\ref{prop:filtrado} (Filtrado a partir del forward) y del Corolario~\ref{cor:causal} (Causalidad del filtrado), Sección~\ref{sec:hmm-filtrado}.
...
Desarrollo de las reestimaciones del paso M del algoritmo de Baum-Welch, Sección~\ref{sec:hmm-em}. La distribución inicial y la matriz de transición se actualizan como frecuencias esperadas,
\[
...
Desarrollo del algoritmo de Viterbi, Sección~\ref{sec:hmm-viterbi}. Si $\delta_t(j) = \max_{z_{1:t-1}} \Prob(z_{1:t-1},\, z_t = j,\, x_{1:t} \mid \lambda)$ es la probabilidad de la mejor trayectoria que termina en el estado $j$ en el instante $t$, la recursión es
\[
...
Demostración del Lema~\ref{lem:em} (Monotonía de EM), Sección~\ref{sec:hmm-em}.
...
Demostración de la Proposición~\ref{prop:bocpd} (Recursión de Adams--MacKay), Sección~\ref{sec:bocpd-recursion}.
...
Demostración de la Proposición~\ref{prop:gbm-residuo} (Pseudo-residuos del boosting logístico), Sección~\ref{sec:sup-gbm}.
...
Demostración de la Proposición~\ref{prop:xgb-score} (Pesos óptimos y ganancia de una división), Sección~\ref{sec:sup-xgboost}.
