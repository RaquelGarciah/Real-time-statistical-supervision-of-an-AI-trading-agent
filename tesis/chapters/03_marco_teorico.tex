...
Una serie de retornos financieros es una realización de un \emph{proceso estocástico} en tiempo discreto: una sucesión de variables aleatorias $\{\reglog\}_{t \ge 1}$ indexada por el tiempo. Modelarla consiste en describir la ley conjunta de esas variables o, de forma más manejable, la \emph{distribución condicional} de cada retorno dado el pasado, $\reglog \mid \mathcal{F}_{t-1}$, donde $\mathcal{F}_{t-1}$ es la información disponible hasta el día anterior. La media y la varianza de esa distribución condicional, $\E[\reglog \mid \mathcal{F}_{t-1}]$ y $\Var(\reglog \mid \mathcal{F}_{t-1})$, son el objeto central del modelado, frente a sus contrapartes \emph{incondicionales}, que promedian sobre todo el horizonte temporal \cite{hamilton1994, tsay2010}.

El punto de partida más simple es el \emph{ruido blanco}: una sucesión de variables incorreladas, de media constante y varianza constante. No tiene estructura temporal explotable en sus dos primeros momentos, y un modelo de varianza condicional constante (\emph{homocedástico}) hereda esa rigidez. Los hechos estilizados de la Sección~\ref{sec:preliminares} lo desmienten: aunque el signo de los retornos sea casi impredecible (poca autocorrelación en niveles), su \emph{magnitud} se agrupa en el tiempo (el agrupamiento de volatilidad), de modo que la varianza condicional no es constante, sino que depende del pasado reciente. Es la \emph{heterocedasticidad condicional}, y un modelo realista de retornos debe capturarla \cite{cont2001}.

Conviene fijar dos nociones de estacionariedad que se usan a lo largo del capítulo. Un proceso es \emph{estrictamente estacionario} si la ley conjunta de cualquier bloque $(r^{\log}_{t_1}, \dots, r^{\log}_{t_k})$ es invariante frente a una traslación común de los índices; es \emph{estacionario en covarianza} (o débilmente estacionario) si su media es constante y su autocovarianza $\operatorname{Cov}(r^{\log}_{t},\, r^{\log}_{t+h})$ depende solo del desfase $h$ y no de $t$ \cite{brockwell1991}. La segunda noción es más débil y es la que caracteriza el Teorema~\ref{teo:garch-estac} para el GARCH. Modelar la varianza condicional como variable en el tiempo permite, de hecho, que el proceso siga siendo estacionario en covarianza: la varianza \emph{incondicional} puede mantenerse constante aunque la \emph{condicional} fluctúe día a día.
