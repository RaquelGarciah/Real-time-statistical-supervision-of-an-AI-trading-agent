...
% >>> CONSEJERO [cap4-03 · estilo]
%   CAMBIARÍA: «La razón es mecánica y se reporta con transparencia:».
%   POR QUÉ:   meta-comentario sobre la propia redacción (correcciones_aprendidas.md → Estilo, prioridad alta: fuera «se reporta con transparencia» y similares). Enuncia el motivo directo, sin anunciar que lo cuentas con honradez.
%   REESCRITURA: «La razón es mecánica:».
% <<< CONSEJERO
...
% >>> CONSEJERO [cap4-01 · coherencia]
%   CAMBIARÍA: «y el Deflated Sharpe Ratio queda en 0,72, por debajo del umbral de habilidad», metido en la batería de contrastes de ACCURACY.
%   POR QUÉ:   el notebook (§9, celda P(Sharpe>0)) reporta como titular P(Sharpe>0)=0,976 y la versión PENALIZADA por las configs ≈0,72, y trata el Sharpe como ILUSTRACIÓN económica, no como prueba (línea roja MANUAL §8: «la prueba del TFG es la accuracy»). Aquí se cae el 0,976, el 0,72 se renombra «DSR» y se mezcla la economía con la significancia direccional. 0,72 es P(Sharpe>0) penalizada por multiplicidad, no un test de dirección.
%   REESCRITURA: saca el Sharpe de esta lista (es economía) y déjalo en el párrafo económico/§9. Quita «; y el Deflated Sharpe Ratio queda en 0,72, por debajo del umbral de habilidad» de esta frase, y añade donde toque: «En lo económico, P(Sharpe>0)=0,976; penalizada por las configuraciones exploradas a lo largo del estudio baja a ≈0,72 (<0,95), por lo que el Sharpe se reporta como ilustración económica y la prueba del trabajo descansa en la accuracy.»
% <<< CONSEJERO
...
La ventaja de M10 no depende del corte concreto de los datos. La Tabla~\ref{tab:smci-robustez} la evalúa sobre tres particiones cronológicas estándar, $60/40$, $70/30$ y $80/20$: en las tres, y tanto en el tramo de validación como en el de test, M10 supera al agente, a la regla y a comprar-y-mantener. La conclusión es estable a las tres particiones estándar probadas, lo que hace improbable que el resultado sea un artefacto de un corte afortunado.
...
% >>> CONSEJERO [cap4-04 · estilo · tentativa]
%   CAMBIARÍA: el cierre «…no se trata de la suerte de un tramo concreto» repite la idea del párrafo anterior («…artefacto de un corte afortunado»).
%   POR QUÉ:   misma conclusión dos párrafos seguidos en §Robustez (redundancia local, correcciones_aprendidas.md → Estructura). Es defendible porque son ejes distintos (partición vs tiempo); si lo dejas, diferencia el cierre para no repetir el mismo remate.
%   REESCRITURA: «…la ventaja se sostiene a lo largo del periodo de evaluación, no en un único tramo.» (y deja «corte afortunado» solo en el párrafo de particiones).
% <<< CONSEJERO
...
% >>> CONSEJERO [cap4-02 · rigor]
%   CAMBIARÍA: «cae al 46,8\%, el nivel del agente».
%   POR QUÉ:   0,468 está por DEBAJO de M5 (0,484), no a su nivel; el notebook (§7b) y MANUAL §3 dan 0,468 sin igualarlo a M5. «El nivel del agente» sugiere una igualdad que no se da (0,468 < 0,484).
%   REESCRITURA: «cae al $46{,}8\%$, en torno al del agente (que acierta el $48{,}4\%$)».
% <<< CONSEJERO
...
La limitación principal es el tamaño y la naturaleza de la muestra. El agente solo existe sobre el periodo posterior al corte del modelo de lenguaje, lo que acota la evaluación a unos $250$ días y vuelve inalcanzable la significancia plena frente a un benchmark justo. Las dos extensiones inmediatas, la generalización a otros agentes y a otros activos, son costosas en inferencia y en tiempo por las razones que se cuantifican en el Capítulo~\ref{cap:conclusiones}, y quedan como trabajo futuro. Lo que sí queda establecido es que el protocolo funciona sobre este activo y es desplegable: en un benchmark honesto, recupera la dirección del agente y supera a la regla a mano y a lo trivial.
