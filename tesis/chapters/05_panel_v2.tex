...
Hay un orden entre las tres. La Figura~\ref{fig:mp-forest} dibuja los intervalos: los tres se mantienen del lado positivo del cero, la regla con su límite inferior pegado al borde y los dos aprendices con holgura. El rescate de riesgo es más firme en los aprendices que en la regla, pero ninguno cruza el cero.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\textwidth]{figuras/cap4_forest_pooled}
\caption{Diagrama de bosque del $\Delta$Sharpe \emph{pooled-10} frente al agente M5, con intervalo al $95\%$ (\emph{bootstrap} estacionario, Politis y Romano 1994). La línea vertical marca el cero. Los tres intervalos excluyen el cero; la regla M8 con el límite inferior más cerca del borde y los aprendices M10 y AutoML con más holgura. Fuente: \texttt{bullbear\_confirmatory.json} (bloque \texttt{POOLED10}).}
\label{fig:mp-forest}
\end{figure}
