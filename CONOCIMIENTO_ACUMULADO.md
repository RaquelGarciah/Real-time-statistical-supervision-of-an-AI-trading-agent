...
> McNemar pareado M8 vs M5: **p ≈ 0.088** (τ=0.40 default). STRATA mejora la accuracy direccional con significancia pareada — **condicional al régimen alcista** (§13 walk-forward: ΔSharpe se invierte en el tramo bajista; falsificación pre-registrada disparada). El resultado honesto es "rescata en este OOS alcista", no rescate universal.
...
> *"Un agente LLM perdedor direccional (38.4%, < azar, sign test p<0.001) es rescatado por supervisión estadística clásica: la accuracy sube 0.384 → 0.436 (regla M8) → 0.539 (XGBoost M10 sobre features STRATA), y regla a mano y caja negra son equivalentes en P&L (DM p=0.67). La señal informativa es la de STRATA: sin features de régimen/RAM/PSA/GSO, M10 cae a Sharpe +0.21 (ablación). Ningún sistema bate B&H pasivo (0.569 accuracy, +32% equity) — STRATA reduce el daño, no genera alfa. El rescate es condicional al régimen alcista (walk-forward §13: ΔSharpe se invierte en el tramo bajista — falsificación pre-registrada disparada); el componente de modelo K=3 sí generaliza inter-época (15/16 orígenes). La aportación es un **protocolo de supervisión estadística interpretable** que recupera accuracy direccional de un agente perdedor y delimita honestamente dónde funciona y dónde no."*
