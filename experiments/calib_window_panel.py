"""Robustez a la ventana de calibración (panel), sugerida por el tutor.

Generaliza smci_calib_window.py a varios activos de historia larga. Recalibra HMM(K=3)+GARCH con distintos
INICIOS de ventana (fin fijo en CALIBRATION_END, anterior al OOS → sin fuga), recomputa régimen/vol y el
walk-forward ensemble de M10, y mide accuracy/Sharpe sobre el OOS FIJO. Reporta la media de retorno por régimen
en cada ventana (para ver si acortar vuelve direccional al régimen).

Pre-registro: ROBUSTEZ, no selección. El OOS no se toca; la ventana completa (2000→2024-09) es la pre-registrada
(CLAUDE.md §3). Se reportan TODAS las ventanas; elegir la que maximiza OOS sería p-hacking. Hipótesis del tutor a
contrastar: "acortar la calibración (el pasado lejano es plano) mejora / vuelve direccional al régimen".

Uso: python experiments/calib_window_panel.py [--tickers SPY,QQQ,XLE,BAC]
"""
...
STARTS = ["2000-01-01", "2010-01-01", "2015-01-01", "2020-01-01"]
OUT = Path("outputs/experiments/calib_window_panel.json")
...
def run_window(tk: str, feat_df, ret, start: str) -> dict:
    msk = (feat_df.index >= pd.Timestamp(start)) & (feat_df.index <= pd.Timestamp(CALIBRATION_END))
    calib = feat_df.loc[msk]
    if len(calib) < 250:
        return {"start": start, "n_cal": int(len(calib)), "skip": "calibración demasiado corta"}
...
def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--tickers", default="SPY,QQQ,XLE,BAC")
    tickers = [t.strip() for t in ap.parse_args().tickers.split(",") if t.strip()]
    config.set_seeds(config.SEED)
    res = {"meta": {"tickers": tickers, "calibration_end": CALIBRATION_END, "oos_start": STRATA_OOS_START,
                    "starts": STARTS, "n_seeds": N_SEEDS, "embargo": EMBARGO, "burn_in": N0,
                    "nota": "robustez a la ventana de calibración (tutor); OOS fijo, sin fuga; ventana completa "
                            "(2000) = pre-registrada (CLAUDE.md §3), NO se elige por el número (no p-hacking)",
    ...
    for tk in tickers:
        ...
        for s in STARTS:
            try:
                r = run_window(tk, feat_df, ret, s)
                res["por_activo"][tk].append(r)
                if "skip" in r:
                    print(f"  {s}: {r['skip']} (n_cal={r['n_cal']})", flush=True)
                else:
                    print(f"  {s} n_cal={r['n_cal']:>5} Crisisμ={r['medias_regimen']['Crisis']:+.5f} "
                          f"M10acc={r['m10_acc']} SR={r['m10_sharpe']:+.2f} | M5={r['m5_acc']} M8={r['m8_acc']}", flush=True)
            except Exception as e:  # noqa: BLE001
                import traceback; traceback.print_exc()
                res["por_activo"][tk].append({"start": s, "error": f"{type(e).__name__}: {e}"})
